...
import type { ImportParsePreview, ImportResult } from "@/lib/types";
...
/**
 * Toàn bộ chức năng nhập khách (2 tab Google Trang tính / Tải file + preview +
 * map cột + commit). Tái dùng được ở cả trang riêng lẫn modal trong trang Khách
 * hàng. `onImported` được gọi sau mỗi lần nhập để màn ngoài refresh danh sách.
 */
export function ImportPanel({
  ...
  return (
    <div>
      ...
      {tab === "google_sheet" ? (
        <GoogleSheetTab onImported={onImported} />
      ) : (
        <FileTab onImported={onImported} />
      )}
    </div>
  );
}
