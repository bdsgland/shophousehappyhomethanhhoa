...
Tài liệu này hướng dẫn anh kết nối **OpenClaw** (`bot.eurowindowlightcity.net`)
với platform ELC để điều khiển toàn bộ hệ thống bằng tiếng Việt qua chat /
...
```
Anh (Telegram CEO bot)  →  OpenClaw (bot.eurowindowlightcity.net)
                                │  gọi HTTP, header X-Openclaw-Token
                                ▼
                    RAI-ELC API  /openclaw/*   (FastAPI agent-engine)
...
3. Username (gợi ý, phải kết thúc bằng `bot`): `pham_van_thu_ceo_bot` hoặc `elc_ceo_bot`
...
2. Paste **cùng một giá trị** vào CẢ 2 service trên Railway:
   - Service **RAI-ELC** (agent-engine) → biến `OPENCLAW_GOD_TOKEN`
   - Service **openclaw** → biến `ELC_API_GOD_TOKEN`
3. **Save** + **Deploy** cả 2 service.

> Nếu `OPENCLAW_GOD_TOKEN` để trống ở RAI-ELC thì toàn bộ `/openclaw/*` trả
> **403** (fail-closed) — đây là cơ chế an toàn, không phải lỗi.

---

## D. Cấu hình OpenClaw trỏ vào ELC API

Trong OpenClaw web UI (`bot.eurowindowlightcity.net`):

1. Đăng nhập OpenClaw.
2. **Settings → Tools → Add new tool**:
   - Type: **HTTP API**
   - Name: `ELC Platform`
   - Base URL: `https://api.eurowindowlightcity.net/openclaw`
   - Header: `X-Openclaw-Token: ${ELC_API_GOD_TOKEN}`
   - Description: `Control toàn bộ ELC platform: users, leads, inventory, commission, analytics, communication`
3. Thêm **system prompt / SOUL.md** cho OpenClaw (xem mục F).

---

## E. (Tuỳ chọn) Các biến môi trường mở rộng trên RAI-ELC
...
```markdown
Bạn là AI Assistant cho Phạm Văn Thư — Giám đốc dự án Eurowindow Light City Thanh Hoá.

Bạn có toàn quyền control platform ELC qua tool "ELC Platform". Khi anh yêu cầu:
...
curl -s https://api.eurowindowlightcity.net/openclaw/kpi/realtime \
...
3. Sinh `openssl rand -hex 32` → paste vào **CẢ 2** service
   (RAI-ELC: `OPENCLAW_GOD_TOKEN`, openclaw: `ELC_API_GOD_TOKEN`) → Deploy.
4. Vào OpenClaw web UI → thêm tool **ELC Platform** (mục D) + SOUL.md (mục F).
